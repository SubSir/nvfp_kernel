...
// SM100 (B200 / GB100) NVFP4 block-scaled GEMM. Mirrors the sm120 path but
// targets the Sm100 tensor-core MMA atoms via the CUTLASS collective builders.
...
// NOTE: do NOT `using namespace std;` here -- it makes `tuple_size_v`
// ambiguous against `cute::tuple_size_v` inside the sm100 epilogue headers.
...
// 1-SM config: good for the small-M (decode) regime.
struct sm100_fp4_config_small {
  using ClusterShape = Shape<_1, _1, _1>;
  using MmaTileShape = Shape<_128, _128, _256>;
};

// 2-SM config: higher throughput for larger M.
...
void cutlass_fp4_gemm_dispatch(torch::Tensor& D, torch::Tensor const& A,
  ...
  if (mp2 <= 256) {
    runGemm<typename Fp4GemmSm100<sm100_fp4_config_small, OutType>::Gemm>(
        D, A, B, A_sf, B_sf, alpha, m, n, k, stream);
  } else {
    runGemm<typename Fp4GemmSm100<sm100_fp4_config_default, OutType>::Gemm>(
        D, A, B, A_sf, B_sf, alpha, m, n, k, stream);
  }
}
...
void cutlass_scaled_fp4_mm_sm100a(torch::Tensor& D, torch::Tensor const& A,
  ...
  TORCH_CHECK(B_sf.sizes()[0] == rounded_n && B_sf.sizes()[1] == rounded_k,
              ...
              "x", rounded_k, "), but got b shape (", B_sf.sizes()[0], "x",
              B_sf.sizes()[1], ")");
  ...
  if (out_dtype == at::ScalarType::BFloat16) {
    cutlass_fp4_gemm_dispatch<cutlass::bfloat16_t>(D, A, B, A_sf, B_sf, alpha, m,
                                                   n, k, stream);
  } else if (out_dtype == at::ScalarType::Half) {
    cutlass_fp4_gemm_dispatch<cutlass::half_t>(D, A, B, A_sf, B_sf, alpha, m, n,
                                               k, stream);
  } else {
    TORCH_CHECK(false, "Unsupported output data type of nvfp4 mm sm100 (",
                out_dtype, ")");
  }
#else
  TORCH_CHECK(false,
              "Unsupported CUTLASS version. Set CUTLASS_DIR to a CUTLASS >= 3.8 "
              "source directory to enable sm100 support.");
