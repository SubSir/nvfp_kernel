...
M = []
d = {"bf16": ([], []), "fp4": ([], []), "resid": ([], [])}
for line in open("/tmp/thr_data2.txt"):
...
fig.suptitle(f"B200 NVFP4 GEMM throughput vs M  (MiniMax-M3 qkv_proj, K={K}, N={N}, CUDA-graph, mean of 60)", fontsize=12)
fig.tight_layout()
fig.savefig("/tmp/throughput_vs_M_mean.png", dpi=140, bbox_inches="tight")
print("saved")
